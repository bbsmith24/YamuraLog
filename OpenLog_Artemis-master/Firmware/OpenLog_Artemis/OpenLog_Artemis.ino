...
=======
  ...
  Serial.println("beginDataLogging");
  beginDataLogging(); //180ms

  Serial.println("beginSerialLogging");
  beginSerialLogging(); //20 - 99ms

  //Serial.println("beginIMU");
  //beginIMU(); //61ms
  ...
  {
    Serial.println("Logging analog pins at max data rate");
  }

  if (settings.enableTerminalOutput == false && settings.logData == true) 
  {
    Serial.println("Logging to microSD card with no terminal output");
  }

  if (beginSensors() == true) 
  {
    Serial.println(beginSensorOutput); //159 - 865ms but varies based on number of devices attached
  }
  else 
  {
