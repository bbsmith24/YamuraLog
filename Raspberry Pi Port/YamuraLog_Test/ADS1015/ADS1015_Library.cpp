...
{
    uint16_t swapVal = (value>>8) | (value & 0xFF);
...
void ADS1015::begin()
{
  ...
  printf("m_i2cFd = %d\n", m_i2cFd);
  i2cDevice.I2CInit(m_i2cAddress);
}
...
*/
/**************************************************************************/
void ADS1015::setI2cAddress(uint8_t i2cAddress) {
  m_i2cAddress = i2cAddress;
}
...
uint16_t ADS1015::readADC_SingleEnded(uint8_t channel) {
  ...
  config.intVal   = ADS1015_REG_CONFIG_CQUE_NONE    | // Disable the comparator (default val)
  ...
  config.intVal |= m_gain;

  // Set single-ended input channel
  switch (channel)
  {
    case (0):
      config.intVal |= ADS1015_REG_CONFIG_MUX_SINGLE_0;
      break;
    case (1):
      config.intVal |= ADS1015_REG_CONFIG_MUX_SINGLE_1;
      break;
    case (2):
      config.intVal |= ADS1015_REG_CONFIG_MUX_SINGLE_2;
      break;
    case (3):
      config.intVal |= ADS1015_REG_CONFIG_MUX_SINGLE_3;
      break;
  }

  // Set 'start single-conversion' bit
  config.intVal |= ADS1015_REG_CONFIG_OS_SINGLE;

  // Write config register to the ADC
  //configWord;
  // reverse bytes
  config.intVal = (config.intVal >> 8) | ((config.intVal << 8) & 0xFF00);
  i2cDevice.I2CWriteBytes(ADS1015_REG_POINTER_CONFIG, config);
  ...
  I2C_Library::I2CWord results;
  i2cDevice.I2CReadBytes(ADS1015_REG_POINTER_CONVERT, &results);
  uint8_t t = results.byteVals[0] ;
  results.byteVals[0] = results.byteVals[1];
  results.byteVals[1] = t;
  return results.intVal >> m_bitShift;
...
void ADS1015::updateWiringPiSetup()
{
  m_i2cFd = wiringPiI2CSetup(m_i2cAddress);
  printf("new fd: %d\n", m_i2cFd);
}
