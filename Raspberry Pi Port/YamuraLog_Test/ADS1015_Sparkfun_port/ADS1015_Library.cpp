...
{
    I2C_Library::I2CWord config;
	if (channel > 3)
	{
		return 0;
	}

	config.intVal = ADS1015_CONFIG_OS_SINGLE   |       // 0x8000         1000 0000 0000 0000
				    ADS1015_CONFIG_MODE_SINGLE |       // 0x0100         0000 0001 0000 0000  ADS1015_CONFIG_MODE_CONT (default)
					ADS1015_CONFIG_RATE_1600HZ |       // 0x0080         0000 0000 1000 0000  ADS1015_CONFIG_RATE_1600HZ (default)
                    ADS1015_CONFIG_PGA_TWOTHIRDS;      // 0x0000         0000 0100 0000 0000  ADS1015_CONFIG_PGA_TWOTHIRDS (default)
                                                       // 0x8180         1000 0100 1000 0000
    config.intVal = 0x8183;
	switch (channel)
    {
    case (0):
        config.intVal |= ADS1015_CONFIG_MUX_SINGLE_0;  // 0x4000    0100 0000 0100 0000
         break;
    case (1):
        config.intVal |= ADS1015_CONFIG_MUX_SINGLE_1;  // 0x5000    0101 0000 0101 0000
        break;
    case (2):
        config.intVal |= ADS1015_CONFIG_MUX_SINGLE_2;  // 0x6000    0110 0000 0110 0000
        break;
    case (3):
        config.intVal |= ADS1015_CONFIG_MUX_SINGLE_3;  // 0x7000    0111 0000 0111 0000
        break;
    }

    uint8_t t = config.byteVals[1];
    config.byteVals[1] = config.byteVals[0];
    config.byteVals[0] = t;

    if(!i2cDevice.I2CWriteBytes(ADS1015_POINTER_CONFIG, config))
    {
        printf("Error writing config register\n");
    }

	delay(ADS1015_DELAY);

	I2C_Library::I2CWord result;
	if(!i2cDevice.I2CReadBytes(ADS1015_POINTER_CONVERT, &result))
	{
	    printf("Error reading conversion register\n");
	}
    t = result.byteVals[1];
    result.byteVals[1] = result.byteVals[0];
    result.byteVals[0] = t;
    //return readRegister(ADS1015_POINTER_CONVERT) >> 4;
    return (result.intVal >> 4);
}
...
int16_t ADS1015::getDifferential(uint16_t CONFIG_MUX_DIFF)
{
	...
	I2C_Library::I2CWord config;
	config.intVal = ADS1015_CONFIG_OS_SINGLE   |
				    _mode |
					_sampleRate;

	config.intVal |= _gain;

    config.intVal |= CONFIG_MUX_DIFF; // default is ADS1015_CONFIG_MUX_DIFF_P0_N1

	i2cDevice.I2CWriteBytes(ADS1015_POINTER_CONFIG, config);
	delay(ADS1015_DELAY);
...
//  _i2cPort->beginTransmission(_i2caddr);
  ...
  uint8_t ptrConvert = ADS1015_POINTER_CONVERT;
  i2cDevice.I2CWriteByte(0, location);//ptrConvert);
  //_i2cPort->endTransmission();
  //_i2cPort->requestFrom((int)_i2caddr, 2); //Ask for one uint8_t
  I2C_Library::I2CWord result;
  i2cDevice.I2CReadBytes(0, &result);
...
//  _i2cPort->beginTransmission(_i2caddr)
    //uint8_t ptrConvert = ADS1015_POINTER_CONVERT;
    //i2cDevice.I2CWriteByte(0, ptrConvert);
//  _i2cPort->write(ADS1015_POINTER_CONVERT);
//  result = _i2cPort->endTransmission();
//  _i2cPort->requestFrom((int)_i2caddr, 2);
  I2C_Library::I2CWord data;
  i2cDevice.I2CReadBytes(location, &data);
...
*/
/**************************************************************************/
void ADS1015::setComparatorSingleEnded(uint8_t channel, int16_t thresh)
{
	if (channel > 3) {
		return;
	}
    I2C_Library::I2CWord config;
    I2C_Library::I2CWord threshold;
    threshold.intVal = thresh << 4;

	config.intVal = ADS1015_CONFIG_MODE_CONT |
					_sampleRate |
    ...
    {
    case (0):
        config.intVal |= ADS1015_CONFIG_MUX_SINGLE_0;
        break;
    case (1):
        config.intVal |= ADS1015_CONFIG_MUX_SINGLE_1;
        break;
    case (2):
        config.intVal |= ADS1015_CONFIG_MUX_SINGLE_2;
        break;
    case (3):
        config.intVal |= ADS1015_CONFIG_MUX_SINGLE_3;
        break;
    }

	// Set the high threshold register
	// Shift 12-bit results left 4 bits for the ADS1015
	i2cDevice.I2CWriteBytes(ADS1015_POINTER_HITHRESH, threshold);
