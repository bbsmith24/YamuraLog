/*
  ...
  based on SparkFun BNO080 library
*/

#include "BNO080_Library.h"
BNO080::BNO080()
{ }
...
bool BNO080::begin(uint8_t deviceAddress, uint8_t intPin)
{
	...
	_intPin = intPin;					//Get the pin that the user wants to use for interrupts. By default, it's NULL and we'll not use it in dataAvailable() function.
	...
	if (receivePacket() == true)
	{
		if (shtpData[0] == SHTP_REPORT_PRODUCT_ID_RESPONSE)
		{
			if (_printDebug == true)
			{
				...
				printf("SW Version Major 0x%02X Minor 0x%02X\n", shtpData[2], shtpData[3]);
				printf("SW Part Number: 0x%04X Build Number: 0x%04X Version Patch: 0x%04X\n", SW_Part_Number, SW_Build_Number, SW_Version_Patch);
			}
			return (true);
		}
	}
...
{
	// I2C
	if(!i2cDevice.I2CReadBytes(_deviceAddress, 0, shtpHeader, 4 ))
	{
        return false;
	}
...
bool BNO080::getData(uint16_t bytesRemaining)
{
	...
	// throw away header
	uint8_t header[4];
	i2cDevice.I2CReadBytes(_deviceAddress, 0, header, 4);
	i2cDevice.I2CReadBytes(_deviceAddress, 0, shtpData, bytesRemaining);
	bytesRemaining = 0;
...
bool BNO080::sendPacket(uint8_t channelNumber, uint8_t dataLength)
{
    ...
    uint8_t packetLenLSB = packetLength & 0xFF;
    uint8_t packetLenMSB = packetLength >> 8;
	i2cDevice.I2CWriteByte(_deviceAddress, 0, packetLenLSB);
	i2cDevice.I2CWriteByte(_deviceAddress, 0, packetLenMSB);
	i2cDevice.I2CWriteByte(_deviceAddress, 0, channelNumber);
	i2cDevice.I2CWriteByte(_deviceAddress, 0, sequenceNumber[channelNumber]);
	sequenceNumber[channelNumber]++;
	//Send the user's data packet
	for (uint8_t i = 0; i < dataLength; i++)
	{
		//_i2cPort->write(shtpData[i]);
		i2cDevice.I2CWriteByte(_deviceAddress, 0, shtpData[i]);
	}
